 ...
 */
const upload = async (credentials, videos, puppeteerLaunch, messageTransport = defaultMessageTransport) => {
    ...
    cookiesFilePath = path_1.default.join(cookiesDirPath, `cookies-${credentials.email.split('@')[0].replace(/\./g, '_')}-${credentials.email
        .split('@')[1]
        .replace(/\./g, '_')}.json`);
    const useCookieStore = !(puppeteerLaunch === null || puppeteerLaunch === void 0 ? void 0 : puppeteerLaunch.userDataDir);
    if (!useCookieStore) {
...
async function uploadVideo(videoJSON, messageTransport) {
    ...
    const errorMessage = await page.evaluate(() => { var _a; return (_a = document.querySelector('.error-area.style-scope.ytcp-uploads-dialog')) === null || _a === void 0 ? void 0 : _a.innerText.trim(); });
    if (errorMessage) {
        await browser.close();
    ...
    if (!videoJSON.isNotForKid) {
        ...
        await page.$eval(`tp-yt-paper-radio-button[name='VIDEO_AGE_RESTRICTION_SELF']`, (e) => e.click());
    }
    else {
...
const update = async (credentials, videos, puppeteerLaunch, messageTransport = defaultMessageTransport) => {
    cookiesDirPath = path_1.default.join('.', 'yt-auth');
    cookiesFilePath = path_1.default.join(cookiesDirPath, `cookies-${credentials.email.split('@')[0].replace(/\./g, '_')}-${credentials.email
        .split('@')[1]
        .replace(/\./g, '_')}.json`);
...
const comment = async (credentials, comments, puppeteerLaunch, messageTransport = defaultMessageTransport) => {
    cookiesDirPath = path_1.default.join('.', 'yt-auth');
    cookiesFilePath = path_1.default.join(cookiesDirPath, `cookies-${credentials.email.split('@')[0].replace(/\./g, '_')}-${credentials.email
        .split('@')[1]
        .replace(/\./g, '_')}.json`);
    ...
    for (const comment of comments) {
        ...
        if (comment.live)
            result = await publishLiveComment(comment, messageTransport);
        else
...
const publishShortComment = async (comment) => {
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            if (comment.pin) {
                ...
                const [commentList] = await page.$x(`//ytd-comments[@id="comments"]//ytd-item-section-renderer[@section-identifier="comment-item-section"]/div[@id="contents"]`);
...
const publishLiveComment = async (comment, messageTransport) => {
    const videoUrl = comment.link;
    const cmt = comment.comment;
    if (!videoUrl) {
        throw new Error('The link of the  video is a required parameter');
...
const updateVideoInfo = async (videoJSON, messageTransport) => {
    ...
    if (description) {
        ...
        // await page.keyboard.up('Control')
        // await page.keyboard.press('Backspace')
        await textBoxes[1].evaluate((e) => (e.__shady_native_textContent = ''));
        await textBoxes[1].type(description.substring(0, maxDescLen));
    }
    if (thumb) {
        const [thumbChooser] = await Promise.all([
            ...
            switch (publish) {
                case 'private':
                    await page
                        ...
                        .catch(async (err) => await page.click(`#privacy-radios > tp-yt-paper-radio-button.style-scope.ytcp-video-visibility-select.iron-selected`));
                    break;
                case 'unlisted':
                    await page
                        .click(`#privacy-radios > tp-yt-paper-radio-button.style-scope.ytcp-video-visibility-select.iron-selected`)
                        .catch(async (err) => await page.click(`#privacy-radios > tp-yt-paper-radio-button:nth-child(11)`));
                    break;
                case 'public':
                    await page
                        .click(`#privacy-radios > tp-yt-paper-radio-button:nth-child(15)`)
                        .catch(async (err) => await page.click(`#privacy-radios > tp-yt-paper-radio-button:nth-child(16)`));
                    break;
...
async function changeLoginPageLangIfNeeded(localPage) {
    ...
    const selectedLang = await localPage.evaluate((selectedLangSelector) => document.querySelector(selectedLangSelector).innerText, selectedLangSelector);
    if (!selectedLang) {
...
async function changeHomePageLangIfNeeded(localPage) {
    ...
    const langMenuItemSelector = '#sections>yt-multi-page-menu-section-renderer:nth-child(3)>#items>ytd-compact-link-renderer:nth-of-type(2)>a';
    try {
    ...
    const selectedLang = await localPage.evaluate((langMenuItemSelector) => document.querySelector(langMenuItemSelector).innerText, langMenuItemSelector);
    if (!selectedLang) {
        throw new Error('Failed to find selected language : Empty text');
    }
    if (selectedLang.includes('English')) {
...
async function scrollTillVeiw(page, element) {
    let sc = true;
    while (sc) {
        try {
...
async function selectGame(page, gameTitle, messageTransport, gameSelector) {
    ...
    await (categoryDropdownToggle === null || categoryDropdownToggle === void 0 ? void 0 : categoryDropdownToggle.click());
    await sleep(1000);
    ...
    for (let i = 0; i < buttonOptions.length; i++) {
        ...
        let testId = await button.evaluate((el) => el.getAttribute('test-id'));
        if (testId == null || !testId.startsWith(`{"title"`))
            continue;
        let gameData = JSON.parse(testId);
